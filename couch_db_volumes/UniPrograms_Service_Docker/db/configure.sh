...
couchbase-cli bucket-create -c 127.0.0.1 --username Administrator --password password --bucket-type couchbase --bucket-ramsize 250 --bucket programs
couchbase-cli bucket-create -c 127.0.0.1 --username Administrator --password password --bucket-type couchbase --bucket-ramsize 250 --bucket universities
couchbase-cli bucket-create -c 127.0.0.1 --username Administrator --password password --bucket-type couchbase --bucket-ramsize 250 --bucket universities_programs
...
cbq -u Administrator -p password -s "CREATE PRIMARY INDEX idx_primary ON \`programs\`;"
...
cbq -u Administrator -p password -s "CREATE PRIMARY INDEX idx_primary ON \`universities\`;"
...
# Create bucket3 indexes
echo "$(date +"%T") Create bucket3 indexes ........."
cbq -u Administrator -p password -s "CREATE PRIMARY INDEX idx_primary ON \`universities_programs\`;"
# cbq -u Administrator -p password -s "CREATE INDEX idx_event_ruleId ON \`bucket1\`(ruleId) WHERE (_type = \"event\");"
# cbq -u Administrator -p password -s "CREATE INDEX idx_event_startTime ON \`bucket1\`(startTime) WHERE (_type = \"event\");"

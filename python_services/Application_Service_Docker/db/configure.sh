...
couchbase-cli bucket-create -c 127.0.0.1 --username Administrator --password password --bucket-type couchbase --bucket-ramsize 250 --bucket schemas
couchbase-cli bucket-create -c 127.0.0.1 --username Administrator --password password --bucket-type couchbase --bucket-ramsize 250 --bucket applications
...
cbq -u Administrator -p password -s "CREATE PRIMARY INDEX idx_primary ON \`schemas\`;"
...
cbq -u Administrator -p password -s "CREATE PRIMARY INDEX idx_primary ON \`applications\`;"
